...
module Qdrant::Api
  # AnyVariants (OpenAPI anyOf): a value matching at least one of the listed schemas.
  ...
  class AnyVariants
    ...
    def initialize(@value : Set(Int64))
    end

    def initialize(@value : Set(String))
    end

    # List of classes defined in anyOf (OpenAPI v3)
    def self.openapi_any_of
      [
        Set(Int64), Set(String),
      ]
    end
    ...
    def self.from_json_any(data : JSON::Any) : self
      json = data.to_json
      begin
        return new(Set(Int64).from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(Set(String).from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      raise JSON::ParseException.new("`#{json}` doesn't match any schema listed in AnyVariants (anyOf)", 0, 0)
    end

    def self.new(ctx : YAML::ParseContext, node : YAML::Nodes::Node)
      begin
        return new(Set(Int64).new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(Set(String).new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      raise YAML::ParseException.new("doesn't match any schema listed in AnyVariants (anyOf)", 0, 0)
    end
