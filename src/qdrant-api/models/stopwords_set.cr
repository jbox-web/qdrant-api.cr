# #Qdrant API
#
# The version of the OpenAPI document: master
# Contact: andrey@vasnetsov.com
# Generated by: https://openapi-generator.tech
# Generator version: 7.24.0
#

module Qdrant::Api
  class StopwordsSet
    include JSON::Serializable
    include YAML::Serializable
    include Qdrant::Api::Serializable
    include Qdrant::Api::Validation

    # Optional properties
    # Set of languages to use for stopwords. Multiple pre-defined lists of stopwords can be combined.
    @[JSON::Field(key: "languages", emit_null: false)]
    property languages : Set(Language)?

    # Custom stopwords set. Will be merged with the languages set.
    @[JSON::Field(key: "custom", emit_null: false)]
    property custom : Set(String)?

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(@languages : Set(Language)? = nil, @custom : Set(String)? = nil)
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      invalid_properties = Array(String).new
      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      list_invalid_properties.empty?
    end

    def_equals_and_hash(languages, custom)
  end
end
