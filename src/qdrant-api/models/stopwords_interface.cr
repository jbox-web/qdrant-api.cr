...
module Qdrant::Api
  # StopwordsInterface (OpenAPI anyOf): a value matching at least one of the listed schemas.
  ...
  class StopwordsInterface
    ...
    def initialize(@value : StopwordsSet)
    end

    # List of classes defined in anyOf (OpenAPI v3)
    def self.openapi_any_of
      [
        Language, StopwordsSet,
      ]
    end
    ...
    def self.from_json_any(data : JSON::Any) : self
      json = data.to_json
      begin
        return new(Language.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(StopwordsSet.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      raise JSON::ParseException.new("`#{json}` doesn't match any schema listed in StopwordsInterface (anyOf)", 0, 0)
    end

    def self.new(ctx : YAML::ParseContext, node : YAML::Nodes::Node)
      begin
        return new(Language.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(StopwordsSet.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      raise YAML::ParseException.new("doesn't match any schema listed in StopwordsInterface (anyOf)", 0, 0)
    end
