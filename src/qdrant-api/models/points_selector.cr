...
module Qdrant::Api
  # PointsSelector (OpenAPI anyOf): a value matching at least one of the listed schemas.
  ...
  class PointsSelector
    ...
    def initialize(@value : FilterSelector)
    end

    def initialize(@value : PointIdsList)
    end
    ...
    def self.from_json_any(data : JSON::Any) : self
      json = data.to_json
      begin
        return new(FilterSelector.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(PointIdsList.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      raise JSON::ParseException.new("`#{json}` doesn't match any schema listed in PointsSelector (anyOf)", 0, 0)
    end

    def self.new(ctx : YAML::ParseContext, node : YAML::Nodes::Node)
      begin
        return new(FilterSelector.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(PointIdsList.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      raise YAML::ParseException.new("doesn't match any schema listed in PointsSelector (anyOf)", 0, 0)
    end
