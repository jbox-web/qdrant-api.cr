# #Qdrant API
#
# The version of the OpenAPI document: master
# Contact: andrey@vasnetsov.com
# Generated by: https://openapi-generator.tech
# Generator version: 7.24.0
#

module Qdrant::Api
  class DistributedPeerInfo
    include JSON::Serializable
    include YAML::Serializable
    include Qdrant::Api::Serializable
    include Qdrant::Api::Validation

    # Required properties
    # URI of the peer
    @[JSON::Field(key: "uri", emit_null: false)]
    property uri : String

    # Whether this peer responded for this request
    @[JSON::Field(key: "responsive", emit_null: false)]
    property responsive : Bool

    # Optional properties
    # If responsive, these details should be available
    @[JSON::Field(key: "details", emit_null: false)]
    property details : DistributedPeerDetails?

    # Initializes the object
    # @param [Hash] attributes Model attributes in the form of hash
    def initialize(@uri : String, @responsive : Bool, @details : DistributedPeerDetails? = nil)
    end

    # Show invalid properties with the reasons. Usually used together with valid?
    # @return Array for valid properties with the reasons
    def list_invalid_properties
      invalid_properties = Array(String).new
      invalid_properties
    end

    # Check to see if the all the properties in the model are valid
    # @return true if the model is valid
    def valid?
      list_invalid_properties.empty?
    end

    def_equals_and_hash(uri, responsive, details)
  end
end
